...
Customer flows (signup, login, catalog, checkout from wallet) are covered there. `/api/me` lists each order with a `fetch` URL. The receipt itself is loaded with `GET /invoices/preview?url=`; the account page calls that and does not render the body. The archive file does not include the line items. It points at an unsigned copy ending in `/body`, loaded through the same preview endpoint. Wallet loads from the card processor arrive as signed billing grants. Workspace coupons are documented in [`promos.md`](promos.md).
