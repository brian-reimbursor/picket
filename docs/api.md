...
Customer flows (signup, login, catalog, checkout from wallet) are covered there. `/api/me` lists each order with a `fetch` URL. The receipt itself is loaded with `GET /invoices/preview?url=` using `orders[].fetch`. View on the account page opens `/invoices/{id}` and only shows that the receipt could not be displayed. Wallet loads from the card processor arrive as signed billing grants. Workspace coupons are documented in [`promos.md`](promos.md).
